#!/usr/bin/env Rscript
# Script: 3_model_caret_spatial_cv.R
# Objective: Fit random forest and XGBoost soil-moisture models with the archived four-fold spatial cross-validation setup.
# Author: Yi Yu
# Created: 2026-04-06
# Last updated: 2026-04-06
# Inputs: Cleaned OzNet predictor tables and site fold assignments.
...
# Usage: Rscript 2_experimental_scripts/2_spatial_sm_prediction/3_model_caret_spatial_cv.R
...
load_site_training_data <- function(site_index, site_info, data_root) {
  ...
  site_df$fold <- site_info$iteration[site_index]
...
fit_and_save_model <- function(caret_method, model_label, predictors_train, response_train, cv_index) {
  ...
  saveRDS(
    fitted_model,
    file = file.path(output_root, paste0(model_label, "_model_caret_4fold_spatial_cv.rds"))
  )
  ...
  saveRDS(
    train_di,
    file = file.path(output_root, paste0(model_label, "_tdi_caret_4fold_spatial_cv.rds"))
  )
}
...
fold_ids <- sort(unique(stats::na.omit(whole_df_train$fold)))
spatial_cv_idx <- lapply(
  fold_ids,
  function(fold_id) {
    which(whole_df_train$fold != fold_id)
  }
)
...
fit_and_save_model("rf", "rf", predictors_train, response_train, spatial_cv_idx)
fit_and_save_model("xgbTree", "xgb", predictors_train, response_train, spatial_cv_idx)
