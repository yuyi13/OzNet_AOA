#!/usr/bin/env Rscript
# Script: 1_data_fusion_ndvi.R
# Objective: Fuse MODIS and Landsat NDVI scenes with ubESTARFM to create daily 100 m NDVI predictors for the OzNet experiments.
# Author: Yi Yu
# Created: 2026-04-06
# Last updated: 2026-04-06
# Inputs: Masked and interpolated MODIS/Landsat NDVI rasters plus the external ubESTARFM implementation.
# Outputs: Fused daily NDVI rasters in /datasets/work/d61-af-soilmoisture/work/model_averaging/1_downscaled_data/ndvi/.
# Usage: Rscript 2_experimental_scripts/1_spatiotemporal_fusion/1_data_fusion_ndvi.R
...
candidate_landsat <- file.path(
  landsat_root,
  format(dates_of_interest, "Landsat_NDVI_NBAR_cloudrm_%Y%m%d.tif")
)
valid_index <- which(has_min_size(candidate_landsat, min_size = 3e6))

candidate_modis <- file.path(
  modis_root,
  format(dates_of_interest[valid_index], "MCD43A4_NDVI_NBAR_cloudrm_%Y%m%d.tif")
)
...
train_landsat <- file.path(
  landsat_interp_root,
  format(valid_dates, "Landsat_NDVI_NBAR_cloudrm_%Y%m%d.tif")
)
train_modis <- file.path(
  modis_root,
  format(valid_dates, "MCD43A4_NDVI_NBAR_cloudrm_%Y%m%d.tif")
)

pred_modis <- list.files(
  modis_root,
  pattern = "^MCD43A4_NDVI_NBAR_cloudrm_\\d{8}\\.tif$",
  full.names = TRUE
)
pred_dates <- as.Date(
